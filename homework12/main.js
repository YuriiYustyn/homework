...
async function getUsers(number) {
   ...
   for (let i = 0; i < response.characters.length; i++) {
      let referencesHaracter = await fetch(`${response.characters[i]}`)
      ...
      foto.src = obj[haracter.name] || `https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS0gJc_0giiMNjbm-kvWcnCjXTJXTM_lCmE7Q&usqp=CAU`
      foto.classList = 'td-foto'

      let tdName = document.createElement('td')
      tdName.innerHTML = haracter.name
      tdName.classList = 'td'

      let tdBirds = document.createElement('td')
      tdBirds.innerHTML = haracter.birth_year
